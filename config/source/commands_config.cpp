...
CommandsConfig::CommandsConfig(const libconfig::Setting& setting) {
  ...
  lookup_scalar_field(init_setting, INITIAL_SEGMENTS_FIELD,
                      init.initial_segments);
  lookup_scalar_field(init_setting, PROMPT_VOLUME_INFO_FIELD,
                      init.prompt_volume_info);
  ...
  lookup_scalar_field(dir_setting, SHOW_VOLUME_INFO_FIELD,
                      dir.show_volume_info);

  const libconfig::Setting& squeeze_setting = setting[SQUEEZE_COMMAND_GROUP];
  lookup_scalar_field(squeeze_setting, DEFRAGMENT_FREE_SPACE_FIELD,
                      squeeze.defragment_free_space);
  lookup_scalar_field(squeeze_setting, VERIFY_AFTER_FIELD,
                      squeeze.verify_after);
}