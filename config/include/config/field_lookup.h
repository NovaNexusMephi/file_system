...
template <typename T>
[[maybe_unused]] inline bool lookup_scalar_field(
    const libconfig::Setting& setting, const std::string& field_name, T& field,
    bool required = false) {
  bool contains_field = setting.exists(field_name);
  if (required && !contains_field) {
    throw std::runtime_error("invalid configuration: " + field_name +
...
template <typename T>
[[maybe_unused]] inline bool lookup_vector_field(
    const libconfig::Setting& setting, const std::string& field_name,
