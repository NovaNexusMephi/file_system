...
INSTANTIATE_TEST_SUITE_P(DefaultSplitTests, SplitTest,
                         testing::ValuesIn(split_test_cases));

class MakeOptTest
    : public testing::TestWithParam<
          std::tuple<std::string, std::pair<std::string, std::string>>> {};
...
const std::vector<std::tuple<std::string, std::pair<std::string, std::string>>>
    make_opt_tests_cases{
        {"", {"", ""}},
        {"option", {"", ""}},
        {":value", {"", ""}},
        {"/option", {"option", ""}},
...
INSTANTIATE_TEST_SUITE_P(DefaultMakeOptTests, MakeOptTest,
                         ::testing::ValuesIn(make_opt_tests_cases));

class NameParamsTest
    : public testing::TestWithParam<
...
INSTANTIATE_TEST_SUITE_P(DefaultNameParamsTests, NameParamsTest,
                         testing::ValuesIn(name_params_test_cases));

class MakeOptObjTest : public testing::TestWithParam<
                           std::tuple<std::vector<std::string>, json>> {};
...
const std::vector<std::tuple<std::vector<std::string>, json>>
    make_opt_obj_test_cases{
        {{}, json::parse("{}")},
        {{"/flag"}, {{"flag", true}}},
        ...
        {{"/enabled", "/count:5", "/title:Test"},
         {{"enabled", true}, {"count", 5}, {"title", "Test"}}},
        {{"/user@domain:admin", "/key:123#456"},
         {{"user@domain", "admin"}, {"key", "123#456"}}},
        {{"invalid_opt", "/:empty", "/valid:ok"}, {{"valid", "ok"}}},
        {{"/:value"}, json::parse("{}")},
        {{"/empty:"}, {{"empty", true}}},
        {{"/num:0123"}, {{"num", 123}}},
        {{"/verbose", "/force", "/recursive"},
         json::parse(R"({"verbose": true, "force": true, "recursive": true})")},
        {{"/setting:5", "/setting:text", "/setting"},
         json::parse(R"({"setting": true})")},
        {{"/server:localhost", "/port:8080", "/ssl", "/timeout:30"},
         json::parse(R"({
   "server": "localhost",
   "port": 8080,
   "ssl": true,
   "timeout": 30
        ...
        {{"/x:10", "/y:20", "/z:30"},
         json::parse(R"({"x": 10, "y": 20, "z": 30})")},
        {{"/valid1", "invalid_opt", "/valid2:ok", "/:empty"},
         json::parse(R"({"valid1": true, "valid2": "ok"})")},
    };

INSTANTIATE_TEST_SUITE_P(DefaultMakeOptObjTests, MakeOptObjTest,
                         testing::ValuesIn(make_opt_obj_test_cases));

class ParseCommandTest : public testing::TestWithParam<
                             std::tuple<std::string, std::optional<json>>> {};
...
const std::vector<std::tuple<std::string, std::optional<json>>>
    parse_command_test_cases{
        {"", std::nullopt},
        {"start", json::parse(R"({
        "name": "start",
        "data": [],
...
INSTANTIATE_TEST_SUITE_P(DefaultParseCommandTests, ParseCommandTest,
                         testing::ValuesIn(parse_command_test_cases));