...
class Segment final {
   public:
    Segment(size_t start, size_t next, size_t size = 0) : header_(start, next, size) {}

    inline void set_next(size_t next) noexcept { header_.next_ = next; }
    ...
    [[nodiscard]] inline size_t get_next() const noexcept { return header_.next_; }
   ...
   private:
    struct SegmentHeader {
        ...
        SegmentHeader(size_t start, size_t next, size_t size) : start_(start), next_(next), counter_(0), size_(size) {}
    };
