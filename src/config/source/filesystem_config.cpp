...
FilesystemConfig::FilesystemConfig(const libconfig::Setting &settings) {
  if (!settings.exists(SYSTEM_INFO_GROUP)) {
    throw std::runtime_error("invalid configuration: missing group " +
                             SYSTEM_INFO_GROUP + ".");
  }
  ...
  segments_number =
      validated_scalar<int>(settings, SEGMENT_NUMBER_FIELD, validation);
#else
  segments_number = scalar_of_default<int>(settings, SEGMENT_NUMBER_FIELD,
                                           lib::DEFAULT_SEGMENT_NUMBER);
#endif
}